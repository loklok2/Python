...
def generate_lotto_tickets(num_tickets=100):       ##로또 100개 디폴드값으로 생성 
    ...
    for _ in range(num_tickets):                       ## 반복문
        ...
        lotto_tickets.append((numbers, bonus_number))      ## 로또랑 보너스번호 리스트에 추가 ((tuple))
...
##당첨확인
def check_lotto_result(winning_numbers, winning_bonus_number, lotto_tickets):   
##winning_numbers는 당첨번호 리스트 winning_bonus_number는 당첨보너스번호  lotto_tickets는 확인할 로또 리스트
